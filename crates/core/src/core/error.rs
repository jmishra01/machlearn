...
pub enum MlError {
    ...
    /// Encoding or decoding a model with `bincode` failed (available under
    /// the `bincode` feature).
    #[error("bincode (de)serialization failed: {0}")]
    SerializationError(String),

    /// Reading or writing a serialized model file failed (available under
    /// the `bincode` feature).
    ...
    /// Reading or converting Arrow data failed.
