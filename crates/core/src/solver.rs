...
/// # Errors
///
/// Returns an error if `design`/`targets` are invalid, mismatched, or
/// non-finite, if `design` is rank-deficient, or if the computed
...
/// # Errors
///
/// Returns an error if `alpha` is negative or non-finite, or if the
/// underlying [`solve_least_squares`] call fails.
pub fn solve_ridge(
    design: ArrayView2<'_, f64>,
    targets: ArrayView1<'_, f64>,
    alpha: f64,
