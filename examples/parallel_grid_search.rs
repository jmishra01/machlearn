//! Rank parameter configurations and folds with optional Rayon execution.

#[cfg(feature = "parallel")]
fn main() -> machlearn::Result<()> {
    use machlearn::{
        Dataset, Fit, KFold, MlError, ParameterGrid, ParameterSet, Predict, ScoreDirection,
        grid_search_parallel, mean_absolute_error,
    };
    ...
    let result = grid_search_parallel(
        &grid,
        configured,
        &dataset,
        &folds,
...
fn main() {
    println!("Run with: cargo run --example parallel_grid_search --features parallel");
}
